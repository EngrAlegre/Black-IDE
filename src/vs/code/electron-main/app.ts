...
export class CodeApplication extends Disposable {
	...
	async startup(): Promise<void> {
		...
		// BLACK IDE: Sidecar Bridge for Python backend
		this.logService.info('Spawning Black IDE Python backend (server.py)...');
		try {
			const { spawn } = await import('child_process');
			const path = await import('path');
			// Path to kimi-agent/server.py
			const pythonScriptPath = path.resolve(this.environmentMainService.appRoot, '../kimi-agent/server.py');
			const pythonProcess = spawn('python', [pythonScriptPath], {
				stdio: 'ignore',
				detached: false
			});
			pythonProcess.on('error', (err: Error) => this.logService.error('Failed to spawn Python backend:', err));
			// Ensure it dies when the IDE dies
			app.on('will-quit', () => {
				try { pythonProcess.kill(); } catch (e) {}
			});
		} catch (err) {
			this.logService.error('Could not spawn Python backend:', err);
		}
