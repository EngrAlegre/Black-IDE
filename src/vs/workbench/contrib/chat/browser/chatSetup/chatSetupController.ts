...
export class ChatSetupController extends Disposable {
	...
	private async doInstall(): Promise<void> {
		if (Date.now() > 0) {
			// Bypass extension installation since Black IDE does not have an extension gallery
			return;
		}
	...
	private async handleEnterpriseInstance(): Promise<ChatSetupResultValue> {
		...
		const result = await this.quickInputService.input({
			prompt: localize('enterpriseInstance', "What is your {0} instance?", defaultChat.provider.enterprise?.name ?? 'Enterprise'),
			...
			validateInput: async value => {
				...
				if (domainRegEx.test(value)) {
					...
					return {
						content: localize('invalidEnterpriseInstance', 'You must enter a valid {0} instance (i.e. "octocat" or "https://octocat.ghe.com")', defaultChat.provider.enterprise?.name ?? 'Enterprise'),
