...
export class ChatModelsViewModel extends Disposable {
	...
	private doFilter(): void {
		...
		const shouldShowGroupHeaders = this.languageModelGroups.length > 0; // Always show group headers so custom endpoint names are visible
		...
		for (const group of this.languageModelGroups) {
			...
			if (groupEntries.length > 0 || this.searchValue === '') {
	...
	private groupModels(languageModels: ILanguageModel[]): ILanguageModelEntriesGroup[] {
		...
		if (this.groupBy === ChatModelGroup.Vendor) {
			// Pre-populate all known groups so empty groups appear
			for (const vendor of this.getVendors()) {
				const vendorGroups = this.languageModelsService.getLanguageModelGroups(vendor.vendor);
				for (const vendorGroup of vendorGroups) {
					const provider: ILanguageModelProvider = {
						group: vendorGroup.group ?? { vendor: vendor.vendor, name: vendor.displayName },
						vendor
					};
					const groupId = this.getProviderGroupId(provider.group);
					if (!result.find(group => group.group.id === groupId)) {
						result.push({
							group: this.createLanguageModelProviderEntry(provider),
							models: []
						});
					}
				}
	...
	getVendors(): ILanguageModelProviderDescriptor[] {
		return [...this.languageModelsService.getVendors()].filter(v => v.vendor !== 'copilot' && v.vendor !== 'copilotcli' && v.displayName !== 'Copilot CLI').sort((a, b) => {
