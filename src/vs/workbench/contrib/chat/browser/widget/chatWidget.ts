...
const DISCLAIMER = localize('chatDisclaimer', "Black may generate inaccurate responses");
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private _getGenerateInstructionsMessage(): IMarkdownString {
		...
		if (this._instructionFilesExist === true) {
			...
			return new MarkdownString(localize(
				...
				"[Configure Black Instructions]({0}) to onboard Black onto your codebase.",
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		...
		if (this.input.currentModeKind === ChatModeKind.Ask) {
			...
			title = localize('agentTitle', "Chat with Black");
		}
	...
	private async _acceptInput(query: { query: string } | undefined, options: IChatAcceptInputOptions = {}): Promise<IChatResponseModel | undefined> {
		...
		// Block request if the resolved model is missing or unresolved
		if (!this.input.currentLanguageModel) {
			this.logService.warn(`[ChatWidget] Blocked request because model state is unresolved or missing.`);
			return;
		}

		// Block request if the outgoing input payload is completely empty
		if (!requestInputs.input.trim() && requestInputs.attachedContext.length === 0) {
			this.logService.warn(`[ChatWidget] Blocked request because the outgoing input/messages array is empty.`);
			return;
		}
