...
export function shouldRestorePersistedModel(
	...
	let model = availableModels.find(m => m.identifier === persistedModelId);

	// If exact match not found, attempt to normalize/remap on provider change.
	// For instance, a stale state might be "Perplexity/sonar" while the new
	// available model is "customendpoint/Perplexity/sonar" or vice-versa.
	if (!model) {
		const parts = persistedModelId.split('/');
		const baseModelName = parts[parts.length - 1];
		if (baseModelName) {
			model = availableModels.find(m => m.identifier.endsWith(`/${baseModelName}`) || m.identifier === baseModelName);
		}
	}
