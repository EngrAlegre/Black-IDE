...
export class LanguageModelsService implements ILanguageModelsService {
	...
	constructor(
		...
		this._vendors.set('Black', {
			vendor: 'Black',
			displayName: 'Black Custom Models',
			isDefault: false,
			managementCommand: 'workbench.action.openSettings',
		} as ILanguageModelProviderDescriptor);

		this._vendors.set('customendpoint', {
			...
			configuration: {
				type: 'object',
				properties: {
					url: { type: 'string', description: 'The custom endpoint URL' },
					apiKey: { type: 'string', description: 'API Key' },
					models: { type: 'array', items: { type: 'string' }, defaultSnippets: [{ body: ['new-model-id'] }] }
				}
			}
		} as unknown as ILanguageModelProviderDescriptor);

		this._vendors.set('customoai', {
			vendor: 'customoai',
			...
			configuration: {
				type: 'object',
				properties: {
					url: { type: 'string', description: 'The OpenAI compatible endpoint URL' },
					apiKey: { type: 'string', description: 'API Key' },
					models: { type: 'array', items: { type: 'string' }, defaultSnippets: [{ body: ['new-model-id'] }] }
				}
			}
