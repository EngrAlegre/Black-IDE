...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Try Black Chat for free, no sign-in required!");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to use Black Chat.");
		...
		const validate = (): boolean => {
			...
			switch (result.kind) {
				...
				case GheParseResultKind.Invalid:
					...
					message.textContent = localize('onboarding.signIn.enterprise.invalid', 'You must enter a valid {0} instance (i.e. "octocat" or "https://octocat.ghe.com")', defaultChat.provider.enterprise?.name ?? 'Enterprise');
					submitAction.enabled = false;
					return false;
			}
		};
	...
	private _renderEnterpriseSignInProgress(actions: HTMLElement): void {
		...
		message.textContent = localize('onboarding.signIn.enterprise.progress', "Waiting for {0} sign-in to complete...", defaultChat.provider.enterprise?.name ?? 'Enterprise');
	}

	private _getEnterpriseInstancePromptLabel(): string {
		return localize('onboarding.signIn.enterprise.prompt', "What is your {0} instance?", defaultChat.provider.enterprise?.name ?? 'Enterprise');
	}
